...
final class OfficerRepository
{
    ...
    public function getLeaderboardPage(int $page, int $pageSize): array
    {
        $page = max(1, $page);
        ...
        $stmt = $this->pdo->prepare(
            'SELECT discord_username, marks FROM officers ORDER BY marks DESC, officer_id ASC LIMIT :limit OFFSET :offset'
        );
    ...
    public function countOfficers(): int
    {
        $stmt = $this->pdo->query('SELECT COUNT(*) AS total FROM officers');
