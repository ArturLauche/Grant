...
final class InteractionHandler
{
    ...
    private function handleMarks(array $interaction): array
    {
        ...
        if ($sub === 'leaderboard') {
            $page = (int) ($this->optionValueFrom($options, 'page') ?? 1);
            $page = max(1, $page);
            $pageSize = 10;

            $totalOfficers = $this->officers->countOfficers();
            if ($totalOfficers <= 0) {
                return $this->message('No officers found for leaderboard.');
            }

            $totalPages = max(1, (int) ceil($totalOfficers / $pageSize));
            if ($page > $totalPages) {
                return $this->message(sprintf('Invalid page. Please choose a page between 1 and %d.', $totalPages));
            }

            $rows = $this->officers->getLeaderboardPage($page, $pageSize);
            if ($rows === []) {
                return $this->message('No officers found for leaderboard.');
            }
            ...
            foreach ($rows as $index => $row) {
                $lines[] = sprintf(
                    '%d. %s — **%d** marks',
                    $startRank + $index,
                    (string) ($row['discord_username'] ?? 'Unknown'),
                    (int) ($row['marks'] ?? 0)
                );
            }

            return $this->message(
                sprintf("Marks Leaderboard (Page %d/%d)
%s", $page, $totalPages, implode("
", $lines))
            );
        }
